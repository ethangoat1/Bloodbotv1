/**
 * @warn Do not edit code or edit credits
 * @author D-Jukie
 * @source Disme Project
 */
module.exports = function ({ api }) {
    const moment = require("moment");
    const botID = api.getCurrentUserID();
    const form = {
        av: botID,
        fb_api_req_friendly_name: "CometNotificationsDropdownQuery",
        fb_api_caller_class: "RelayModern",
        doc_id: "5025284284225032",
        variables: JSON.stringify({
            "count": 5,
            "environment": "MAIN_SURFACE",
            "menuUseEntryPoint": true,
            "scale": 1
        })
    };
    try {
        api.httpPost("https://www.facebook.com/api/graphql/", form, (e, i) => {
            if (e || !i) return;
            try {
                var a = JSON.parse(i);
                if (!a || !a.data || !a.data.viewer) return;
                var data = a.data.viewer;
                const get_minutes_of_time = (d1, d2) => {
                    let ms1 = d1.getTime();
                    let ms2 = d2.getTime();
                    return Math.ceil((ms2 - ms1) / (60 * 1000));
                };
                for (let item of data.notifications_page.edges) {
                    if (item.node.row_type !== 'NOTIFICATION') continue;
                    var audio = data.notifications_sound_path ? data.notifications_sound_path[1] : null;
                    var count = data.notifications_unseen_count;
                    var body = item.node.notif.body.text;
                    var link = item.node.notif.url;
                    var timestemp = item.node.notif.creation_time.timestamp;
                    var time = moment.tz("Asia/Ho_Chi_minh").format("HH:mm:ss DD/MM/YYYY");
                    if (get_minutes_of_time(new Date(timestemp * 1000), new Date()) <= 1) {
                        var msg = "" + 
                            "=== 『 🌸 𝗡𝗢𝗧𝗜𝗙𝗜𝗖𝗔𝗧𝗜𝗢𝗡 🌸 』 ===" +
                            "\n\n『⏱️』→ 𝗧𝗶𝗺𝗲: " + time + 
                            "\n『💬』→ 𝗠𝗲𝘀𝘀𝗮𝗴𝗲: " + body +
                            "\n『🔗』→ 𝗟𝗶𝗻𝗸:\n" + link;
                        api.sendMessage(msg, global.config.ADMINBOT[0]);
                    }
                }
            } catch (error) {
                // Ignore errors
            }
        });
    }
    catch(e) {
        console.log(`Đã xảy ra lỗi khi gửi thông báo: ${e}`);
    }
}
